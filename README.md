...
- 393 个测试，含 **安全红线测试**（仅 A/B 可清理项可删且只进回收站、C-E/容器/黑名单/占用/symlink 必拒、手动高风险覆盖仍不能碰黑名单/容器/占用 /

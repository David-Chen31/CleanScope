...
> AI 辅助的 Windows C 盘清理分析工具 —— **只解释，不替你删**。

CleanScope 扫描你的磁盘，按来源、归属、依赖和风险把文件/目录讲清楚，给出分级建议，
**把最终删除决定权完全交还给你**。它不替你删除任何重要文件，而是优先解释“这是什么、属于谁、删了会怎样”。

## 四条产品宪法（设计红线）

1. **AI 不直接删除 C 盘重要文件** —— AI 只做建议，永不触碰磁盘。
2. **优先解释**文件的来源 / 依赖 / 风险，而非直接处理。
3. **用户做最终删除决定**。
4. 产品必须**安全、可解释、可测试、可扩展**。

当前 MVP 为**零删除**版本：唯一可改盘的安全闸门对一切删除意图恒拒，代码库里**根本不存在永久删除 API**
（有静态测试 + CI 门禁守着）。它现在是一个**只读分析器**：扫描 → 取证 → 规则 → 归因 → 风险 → 决策 → 报告。
...
2. **文件清单** —— 路径 / 大小 / 归属 / 风险 / 建议，可按大小或风险排序；**不渲染任何删除按钮**。
3. **文件详情** —— 属性、风险评估、**证据链（事实证据 vs AI 推测，视觉区分）**、归因候选、AI 解释；
   高风险（D/E）仅提示“不建议删除”，无删除入口；页内有只读的“安全闸门判定”演示——对删除意图必拒。
   仅提供安全辅助操作：打开所在文件夹 / 复制路径 / 加入忽略名单。
4. **报告 / 忽略名单** —— 导出 Markdown 报告；管理全局忽略名单（增删，仅本地存储）。
...
- 162 个测试，含 **T-01 ~ T-20 安全红线测试**（删除恒拒 / 黑名单 / 占用 / symlink 防绕过 /
  无永久删除 API / AI 不放低风险 / 脱敏出云 / 审计先写后执行 …）。
...
src/
  ...
  CleanScope.Safety          安全闸门（唯一可改盘路径）+ 执行器（无删除代码）
...
- **代码库中不存在永久删除 API**（有静态扫描测试断言）。
- 唯一可改盘路径是安全闸门；MVP 对一切删除意图恒拒。
- AI 不能绕过规则引擎、不能调低风险；无法判断时输出“无法判断，不建议删除”。

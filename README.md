...
Classification knowledge lives as **declarative data** in [`rules/`](rules/) (12 packs, 59 rules), not hardcoded. Extend recognition by editing JSON, no code changes needed. System-critical directories are forced into a non-deletable blacklist in `00-system-critical.json`.
...
分类知识以**声明式数据**形式存放在 [`rules/`](rules/)（12 个包、59 条规则），而非硬编码。
